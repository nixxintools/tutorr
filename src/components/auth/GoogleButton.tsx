...
export function GoogleButton({ label = "Continue with Google", variant = "default" }: GoogleButtonProps) {
  ...
  return (
    <div className="w-full">
      ...
      {error && (
        <p className="mt-2 text-center text-xs" style={{ color: "rgba(252,165,165,1)" }}>{error}</p>
      )}
    </div>
  );
}
